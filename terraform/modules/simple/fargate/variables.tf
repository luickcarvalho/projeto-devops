...
variable "fargate_spot_base" {
  description = "The number of task to run on FARGATE SPOT that must run all the time. Note: Changing this number after creation will destroy and recreate the service (downtime)."
  default     = 1
  type        = number
}

variable "fargate_spot_weight" {
  description = "The proportional tasks to run on FARGATE SPOT. Note: Changing this number after creation will destroy and recreate the service (downtime)."
  default     = 4
  type        = number
}

variable "fargate_base" {
  description = "The number of task to run on FARGATE that must run all the time. Note: Changing this number after creation will destroy and recreate the service (downtime)."
  default     = 0
  type        = number
}

variable "fargate_weight" {
  description = "The proportional tasks to run on FARGATE. Note: Changing this number after creation will destroy and recreate the service (downtime)."
  default     = 1
  type        = number
}
...
variable "target_group_arns" {
  description = "List of target group ARNs for the service"
  type        = list(string)
  default     = []
}

variable "elb_names" {
  description = "List of ELB names for the service"
  type        = list(string)
  default     = []
}
